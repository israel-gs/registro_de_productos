...
public class SupplierProvider {
    ...
    public Either<String, String> insertSupplier(SupplierModel supplier) {
        try {
            int result = query.update("INSERT INTO supplier (id, name, ruc, phone, email) VALUES ('" + supplier.getID() + "', '" + supplier.getName() + "', '" + supplier.getRuc()+ "', '" + supplier.getPhone()+ "', '" + supplier.getEmail()+ "');");
    ...
    public Either<String, String> updateSupplier(SupplierModel supplier) {
        try {
            int result = query.update("UPDATE supplier t SET t.name = '" + supplier.getName() + "', t.ruc = '" + supplier.getRuc()+ "', t.phone = '" + supplier.getPhone()+ "', t.email = '" + supplier.getEmail()+ "' WHERE t.id LIKE '" + supplier.getID() + "';");
            return switch (result) {
                case 1 ->
